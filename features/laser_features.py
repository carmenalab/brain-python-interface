...
class ArduinoGPIO(object):
    ''' Pin-addressable arduino serial interface. Very slow'''
    def __init__(self, port=None, baudrate=500000, timeout=10, line_end='\n'):
        if port is None:
        ...
        self.version = self.cmd('version')

    def cmd(self, cmd, resp=False):
        cmd += self.line_end
    ...
    def write(self, pin: int, value: bool):
        return self.cmd('write ' + str(pin) + ' ' + str(int(value)))
    
    def read(self, pin):
        return bool(self.cmd('read ' + str(pin), resp=True))
