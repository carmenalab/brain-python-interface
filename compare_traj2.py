...
idx_aim     = traj_pbk[trial_type]['task']['idx_aim']
idx_aim_psi = traj_pbk[trial_type]['task']['idx_aim_psi']
plant_pos   = traj_pbk[trial_type]['task']['plant_pos']
command_vel = traj_pbk[trial_type]['task']['command_vel']
idx_traj    = traj_pbk[trial_type]['task']['idx_traj']
...
if armassist:
    ...
    plt.plot(rad_to_deg * time_warped_ref_psi_traj, '-D', color=color_ref, markersize=2.5)
    plt.plot(rad_to_deg * plant_pos[:,2],           '-D', color=color_pbk, markersize=2.5)
    ...
    for idx, (plant_psi, aim_psi, cmd_vel_psi) in enumerate(zip(plant_pos[:,2], aim_pos[:,2], command_vel[:,2])):
        ...
        plt.plot([idx, idx], [rad_to_deg*plant_psi, rad_to_deg*aim_psi],                   color='green', markersize=2.5)
        ## line showing command velocity
        #plt.plot([idx, idx], [rad_to_deg*plant_psi, rad_to_deg*(plant_psi + cmd_vel_psi)], color='gray',  markersize=2.5)
    ...
    for i, state in enumerate(aa_pos_states):
        ...
        if state == 'aa_ppsi':
            ...
            scale = 1
        plt.plot(scale * aa_ref[state][idx_traj.reshape(-1)], color=color_ref)
        plt.plot(scale * plant_pos[:, i], 'D', color=color_pbk, markersize=2.5)
    ...
    ax = plt.subplot2grid(grid, (0, 0))
    ax.set_title('command xy speed during playback')
    ...
    for i, state in enumerate(aa_pos_states):
        ...
        ax.set_title(state + ' command velocity during playback')
        if state == 'aa_ppsi':
            scale = rad_to_deg    
        else:
            scale = 1
        plt.plot(scale * command_vel[:, i], color=color_pbk)
...
if rehand:
    ...
    for i, state in enumerate(rh_pos_states):
        ax = plt.subplot2grid(grid, (i, 0))
        ax.set_title(state + ' trajectories (time-warped)')
        plt.plot(rad_to_deg * rh_ref[state][idx_traj.reshape(-1)], color=color_ref)
        plt.plot(rad_to_deg * plant_pos[:, i+offset], color=color_pbk)
