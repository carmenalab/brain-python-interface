...
class Track(singleton.Singleton):
    ...
    def runtask(self, **kwargs):
        ...
        self.proc = TaskWrapper2(log_filename=log_filename, **kwargs)
        self.task_proxy = self.proc.start()
...
def remote_runtask(tracker_end_of_pipe, task_end_of_pipe, websock, **kwargs):
    ...
    cmd_proxy = RPCWrapper(task_end_of_pipe)
    ...
    try:
        ...
        while (cmd is not None) and (task_wrapper.task.state is not None):
            proc_status = cmd_proxy.proc_command(cmd)
            if proc_status == "completed":
                cmd = cmd_proxy.pipe.recv()
            elif proc_status == "error":
                if cmd_proxy.pipe.poll(60.):
                    cmd = cmd_proxy.pipe.recv()
                else:
                    cmd = None
...
class TaskObjProxy(ObjProxy):
    def end_task(self):
        end_task_fn = FuncProxy("end_task", self.pipe)
        end_task_fn()
        self.send_terminate_signal()

    def remote_set_attr(self, attr, value):
        self.log_str('trying to remotely set attribute %s to %s\n' % (attr, value))
        set_task_attr = FuncProxy('set_task_attr', self.pipe)
        set_task_attr(attr, value)


class TaskWrapper2(RPCProcess):
    '''
    Wrapper for Experiment classes launched from the web interface
    '''
    proxy = TaskObjProxy

    def __init__(self, subj, base_class, feats, params, websock=None, status=None,
    ...
    def run(self):
        ...
        # Force all tasks to use the Notify feature defined above. 
        if use_websock:
            self.feats.insert(0, websocket.NotifyFeat)
            self.params.params['websock'] = self.websock
        self.params.params['tracker_status'] = self.status
        # kwargs['params']['tracker_end_of_pipe'] = tracker_end_of_pipe


        Task = experiment.make(self.base_class, feats=self.feats)
        print(self.feats)
        self.params.trait_norm(Task.class_traits())
        ...
        self.status = "running" if saveid is None else "testing"        

        self.log_str("Initial task state: %s" %self.task.state)
        ret_status, msg = super().run()
    ...
    def check_run_condition(self):
        return self.task.state is not None
