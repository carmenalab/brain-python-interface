...
try:
    ...
    plexlib.plx_open.argtypes = [C.c_char_p]
    plexlib.plx_load.argtypes = [C.POINTER(PlexFile), C.c_bool]
...
class DataFile(object):
    def __init__(self, filename):
        ...
        self.length = self.plxfile.contents.length
        self.freq = self.plxfile.contents.header.ADFrequency
        finalizer.track_for_finalization(self, self.plxfile, self._free)

    def load(self, recache=False):
        plexlib.plx_load(self.plxfile, recache)
    ...
    def get_units(self):
        units = []
        for i, chan in enumerate(self.plxfile.contents.chan_info):
            for u in range(chan.NUnits):
                units.append((i+1, u+1))
                
        return units

def openFile(filename, load=True, recache=False):
