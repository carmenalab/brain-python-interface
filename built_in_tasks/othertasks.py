...
from features.laser_features import ArduinoGPIO, DigitalWave
from riglib.experiment import traits
import itertools
import numpy as np

class LaserExperiment(Sequence):
    ...
    def _start_wait(self):
        ...
        wave.start()
        super(LaserExperiment, self)._start_wait()
    ...
    @staticmethod
    def pulse(nreps=100, duration=[0.005], power=[1]):
        '''
        ...
        dur_seq, pow_seq = LaserExperiment.gen_random_conditions(nreps, duration, power)
        edge_seq = map(lambda dur: [0, dur], dur_seq)
        return list(zip(pow_seq, edge_seq))

    @staticmethod
    def square_wave(nreps=100, freq=[20], duration=[0.005], power=[1]):
        '''
        ...
        duration = make_list_of_float(duration)
        power = make_list_of_float(power)
        freq_seq, dur_seq, pow_seq = LaserExperiment.gen_random_conditions(nreps, freq, duration, power)
